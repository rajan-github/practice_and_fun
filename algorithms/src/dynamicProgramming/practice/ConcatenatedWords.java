...
public class ConcatenatedWords {
	public static List<String> findAllConcatenatedWordsInADict(String[] words) {
		...
		Set<String> concatenatedWordsSet = new HashSet<>();

		for (String word : words) {
			if (word.length() >= 2 * minLength && word.length() > 1
					&& isConcatenatedWord(word, allWords, concatenatedWordsSet, minLength, false)) {
				concatenatedWords.add(word);
			} else {
//				System.out.println(word + " is not concatenated.");
			}
		}
		return concatenatedWords;
	}

	private static boolean isConcatenatedWord(String word, Set<String> allWords, Set<String> concatenatedWords,
			int minLength, boolean isBroken) {
		if (concatenatedWords.contains(word) || (isBroken && allWords.contains(word)))
			return true;
		else if (word.length() < 2 * minLength && word.length() > 0)
			return false;
		...
		for (int i = 0; i < wordLength - 1; i++) {
			...
			substr.append(c);
			if (allWords.contains(substr.toString())
					&& isConcatenatedWord(word.substring(i + 1), allWords, concatenatedWords, minLength, true)) {
				concatenatedWords.add(word);
				return true;
			}
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(findAllConcatenatedWordsInADict(new String[] { "db", "a", "b", "c", "ab", "abc", "db" }));

		System.out.println(findAllConcatenatedWordsInADict(new String[] { "rfkqyuqfjkx", "", "vnrtysfrzrmzl", "gfve",
				"qfpd", "lqdqrrcrwdnxeuo", "q", "klaitgdphcspij", "hbsfyfv", "adzpbfudkklrw", "aozmixr", "ife",
				"feclhbvfuk", "yeqfqojwtw", "sileeztxwjl", "ngbqqmbxqcqp", "khhqr", "dwfcayssyoqc", "omwufbdfxu",
				"zhift", "kczvhsybloet", "crfhpxprbsshsjxd", "ilebxwbcto", "yaxzfbjbkrxi", "imqpzwmshlpj", "ta",
				"hbuxhwadlpto", "eziwkmg", "ovqzgdixrpddzp", "c", "wnqwqecyjyib", "jy", "mjfqwltvzk", "tpvo",
				"phckcyufdqml", "lim", "lfz", "tgygdt", "nhcvpf", "fbrpzlk", "shwywshtdgmb", "bkkxcvg", "monmwvytby",
				"nuqhmfj", "qtg", "cwkuzyamnerp", "fmwevhwlezo", "ye", "hbrcewjxvcezi", "tiq", "tfsrptug",
				"iznorvonzjfea", "gama", "apwlmbzit", "s", "hzkosvn", "nberblt", "kggdgpljfisylt", "mf", "h",
				"bljvkypcflsaqe", "cijcyrgmqirz", "iaxakholawoydvch", "e", "gttxwpuk", "jf", "xbrtspfttota",
